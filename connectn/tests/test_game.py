...
def test_check_end_state_diagonal(empty_board):
    from connectn.game import check_end_state
    for i in range(CONNECT_N):
        n_diagonal = np.diag(np.ones(CONNECT_N-i, dtype=btype))
        for n_conn in (n_diagonal, n_diagonal[:, ::-1]):
            for row in range(empty_board.shape[0]-CONNECT_N+i+1):
                for col in range(empty_board.shape[1]-CONNECT_N+i+1):
                    for player in (PLAYER1, PLAYER2):
                        b0 = empty_board.copy()
                        b0[row:row+CONNECT_N-i, col:col+CONNECT_N-i] = player*n_conn
                        if i == 0:
                            assert check_end_state(b0, player) == IS_WIN
                        else:
                            assert check_end_state(b0, player) == STILL_PLAYING
...
def test_generate_move_process(empty_board):
...
def test_run_games(monkeypatch):
    ...
    updated_agents_b = [('agent_columns', 'no file'),
                        ('agent_random', 'no file')]
    for updated_agents in (updated_agents_a, updated_agents_b):
        ...
        monkeypatch.setattr(utils, 'update_user_agent_code', partial(mock_update_user_agent_code, updated_agents))
